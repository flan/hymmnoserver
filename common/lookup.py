...
def initialiseEmotionVerbRegexps(db_con):
    ...
    if not EMOTION_VERB_REGEXPS:
        ...
        EMOTION_VERB_REGEXPS = tuple([(
            re.compile(r"^{emotion_vowels_full}(eh|aye|za)?$".format(
...
def _readWord(word, dialect, db_con):
    ...
    if dialect:
        limiter = "AND dialect = {dialect}".format(
            dialect=dialect,
        )
        
    cursor = db_con.cursor()
    cursor.execute("SELECT word, meaning, kana, class, dialect, syllables FROM hymmnos WHERE word = ? " + limiter, (word,))
    records = cursor.fetchall()
    cursor.close()
