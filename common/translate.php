...
<?php
    ...
    $emotion_verbs = array();
    $stmt = $db->prepare("SELECT word FROM hymmnos WHERE class = 1");
    ...
    while(true){#Display every result.
        ...
        $emotion_verbs[] = array(
            '/^'.str_replace('.', $EMOTION_VOWELS_REGEXP_FULL, $stmt_result_row['word']).'(eh|aye|za)?$/',
            substr_count($stmt_result_row['word'], '.'),
    ...
    function queryWord($word){
        ...
        global $db;
        $stmt = $db->prepare('SELECT word, meaning, kana, class, dialect, COUNT(*) AS hits FROM hymmnos WHERE word = :word ORDER BY dialect ASC LIMIT 1');
        $stmt->bindValue(':word', $word, SQLITE3_TEXT);
        $stmt_result = $stmt->execute();
        if($stmt_result->numColumns() && $stmt_result->columnType(0) != SQLITE3_NULL){
            $stmt_result_row = $stmt_result->fetchArray(SQLITE3_ASSOC);
